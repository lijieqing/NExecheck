...
/**
 * adapter for datauploadactivity{@linkplain DataUploadActivity}.
 */
public class DataUploadListAdapter extends BaseAdapter {

	private List<CheckRecordEntity> data;
	private LayoutInflater layoutInflater;

	/**
	 * Instantiates a new Data upload list adspter.
	 *
	 * @param context the context
	 * @param data    the data
	 */
	public DataUploadListAdapter(Context context, List<CheckRecordEntity> data){
		this.data=data;
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		if(convertView==null){
			...
			///获得组件，实例化组件
			convertView=layoutInflater.inflate(R.layout.data_upload_list, null);
			...
			crList.checkRecordCB=(CheckBox)convertView.findViewById(R.id.checkRecordCB);
			convertView.setTag(crList);
		}else{
			crList=(CheckRecordList)convertView.getTag();
		}
		//绑定数据
		crList.finishTimeTV.setText(data.get(position).getFinishTime());
		crList.deviceNameTV.setText(data.get(position).getDeviceName());
		crList.subdeviceNameTV.setText(data.get(position).getSubdeviceName());
		crList.excIdTV.setText(data.get(position).getExcId());
		crList.checkStatusImage.setText(CheckRecordStatusEnum.getName(data.get(position).getCheckStatus()));
		crList.checkerNameTV.setText(data.get(position).getCheckerName());
		crList.checklineNameTV.setText(data.get(position).getChecklineName());
		crList.checkRecordCB.setChecked(data.get(position).getCheckBoxState());
		return convertView;
	}
}
